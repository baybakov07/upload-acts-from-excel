...
# Ищем проверки без замечаний, где нет ссылок на нарушения
df_test["Выявленные нарушения по технологии работ"] = df_test["Выявленные нарушения по технологии работ"].fillna("Отсутствуют")
df_test = df_test[~df_test['Выявленные нарушения по технологии работ'].str.contains(r"\(.*п\.\d+\)|\(.*п \d+\)|\(.*п\s*\d+\)|\(.*п\. \d+.*\)")]
...
df_not_find.to_excel(f"Ненайденные данные/Ненайденные данные в проверке без замечаний.xlsx", index=False)
...
df_test = df_test[["dateAkt", "superFIO", "padstep1", "well", "field", "podrCompanystep1", \
                   "MasterFIO", "numBrigade", "zaknamestep1_content", "zaknamestep1"]]
...
df_new_for_content = df_test.copy()
...
df_new_raw_content = df_test.copy()
...
df_test['dateAktmain'] = df_test['dateAkt'].apply(
...
df_final["name"] = df_test['dateAktmain'].apply(lambda x: "Проверка без замечания_sign_new. " + x)
...
#     data_final = df_final.loc[i].to_dict()
#     try:
